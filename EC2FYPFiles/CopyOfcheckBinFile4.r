...
checkBin<-function(f,l,N){
  ...
  while(i<((l-f)/2)){
    ...
    round(normalisedFourierComponents[i], digits = 6);
    Re(normalisedFourierComponents[i]);
    cat(normalisedFourierComponents[i]);
    cat("\n");
    #if cannot be evaluated
    if(is.na(normalisedFourierComponents[i])) {cat(" Unknown");cat("\n")
    #if the real part of the complex number is greater than the threshhold
    #print "Over" to the console
    } else if ((normalisedFourierComponents[i])>0.02){cat(" Over");cat("\n")
